...
exports.run = async (client, message, args) => {
  ...
  member.setNickname(`${client.config.tag2} ${isim} | ${yaş}`)
      ...
      const memeniaç = new Discord.MessageEmbed()
    .setAuthor(message.author.username, message.author.displayAvatarURL({dynamic: true}))
    .setDescription(`${client.config.murphyTag2} ${member} kişisinin ismi başarıyla \`"${isim} | ${yaş}"\` olarak değiştirildi.\n\nKişinin önceki isimlerine \`!isimler @Kullanıcı/ID\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.`)
    .setFooter(`${client.config.footer}`)
    .setColor("RANDOM")
    message.channel.send(memeniaç);
