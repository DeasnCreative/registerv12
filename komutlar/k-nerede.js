...
exports.run = async (client, message, args) => {
  ...
  let selfD = uye.voice.selfDeaf ? "🔇" : "🔊";
   ...
   if(!uye) return message.channel.send(new Discord.MessageEmbed().setDescription('Doğru kullanımı: ```!nerede @Kullanıcı/ID```'));
...
let number = channel.members.size;  
 let murphyembed = new Discord.MessageEmbed()
   ...
   if(channel.members.array().length > 1) a = `Onunla birlikle olanlar [${number}] : `+channel.members.map((user) => { if (user === channel.member) return; return user; })
     message.channel.send(murphyembed.setFooter(`${client.config.footer}`).setDescription(`**${uye} \`${message.guild.channels.cache.get(uye.voice.channelID).name}\` adlı ses kanalında mikrofonu ${selfM}, kulaklığı ${selfD} \n ${a || `Odada sadece ${uye} bulunmaktadır.`} **`))
 }
