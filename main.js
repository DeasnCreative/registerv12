...
client.login(process.env.SUDETOKEN);
...
client.on("guildMemberAdd", member => {
 ...
 member.roles.add(`${client.config.kayıtsız}`)
     ...
     const kanal = member.guild.channels.cache.find(r => r.id === `${client.config.welcomechannel}`);
   ...
   if (kurulus > 1296000000) kontrol = '**Güvenilir gözüküyor!** ✅'
     moment.locale("tr");
...
Sunucu kurallarımız <#${client.config.rules}> kanalında belirtilmiştir. Unutma sunucu içerisindeki \`ceza-i işlemlerin\` kuralları okuduğunu varsayarak yapılacak!

Sunucumuz seninle beraber **${member.guild.memberCount}** kişi oldu!

Tag alarak ailemizin bi parçası olabilirsin, İyi eğlenceler! ||<@&${client.config.teyitci}>||
`)
});
...
client.on("ready", async () => {
      client.user.setActivity("ashriel cnm", 
        { url: 'https://twitch.tv/.',
        type: 'STREAMING' }); 
})
...
if (newMessage.author.bot || newMessage.channel.type == "dm") return;
    let kanal = `${client.config.mesajlog}` 
  if (!kanal) return;
